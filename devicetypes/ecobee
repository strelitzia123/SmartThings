...
*/        
		...
		standardTile("weatherPop", "device.weatherPop", inactiveLabel: false, 
 			width: 2,height: 2,  decoration: "flat", canChangeIcon: false) {
			state "default", label: 'Rain/Snow Prob. ${currentValue}%', unit: "%",
			icon: "st.Weather.weather2",
 			backgroundColor: "#ffffff"
		}
...
void setTemperature(value) {
	...
	if (getTemperatureScale() == 'C') { 		
		...
		if (settings.trace) {
			log.debug "setTemperature in Celsius>after temp correction= $temp vs. state.currentTileValue=$state.currentTileValue"
		}	        
		...
		if (settings.trace) {
			log.debug "setTemperature in Farenheit>after temp= $temp vs. state.currentTileValue=$state.currentTileValue"
		}	        
		evaluate(temp, heatTemp, coolTemp)
		state?.currentTileValue=temp	        
	}        
...
private def refresh_thermostat(thermostatId) {
	...
	if (settings.trace) {
		...
		if (foundEvent) {
			...
			sendEvent name: "verboseTrace", value:
				"poll>thermostatId = ${thermostatId},event's holdClimateRef =${foundEventClimateRef}"
		}
	}
...
//		groups: (ecobeeType.toUpperCase() == 'REGISTERED')? getThermostatGroups(thermostatId) : 'No groups',
	...
	evaluate(device.currentValue("temperature"), device.currentValue("heatingSetpoint"), device.currentValue("coolingSetpoint"),
		true)   
	sendEvent name: "verboseTrace", displayed: settings.trace, value:
...
// 			Speed variable names contain 'speed'
				...
				sendEvent(name: name, value: value, displayed: settings.trace)                                     									 
	...
	for (i in 0..data.thermostatList.size() - 1) {
		...
		while ((statusCode) && (j++ <2)) { // retries once if api call fails
			...
			api('setHold', bodyReq) {resp ->
				...
				if ((resp.status == 500) && (statusCode == ECOBEE_NEED_TOKEN_REFRESH)) {
					if (settings.trace) {
						log.debug "setClimate>thermostatId=${data.thermostatList[i].identifier}, error $statusCode, need to call refresh_tokens()"
						sendEvent name: "verboseTrace", value:
							"setClimate>thermostatId=${data.thermostatList[i].identifier},error $statusCode, need to call refresh_tokens()"
					}
					refresh_tokens()     
				}
				exceptionCheck=device.currentValue("verboseTrace")
	...
	while ((statusCode) && (j++ <2)) { // retries once if api call fails
		...
		api('runtimeReport', bodyReq) {resp ->
			...
			if (!statusCode) {
				...
				sendEvent name: "verboseTrace", displayed: settings.trace
...
private int get_interval(Date dateTime) {
	...
	if (settings.trace) {
		...
		sendEvent name: "verboseTrace", value: "get_interval> Calendar hour= ${c.get(Calendar.HOUR_OF_DAY)}"
		sendEvent name: "verboseTrace", value: "get_interval> Calendar minute= ${c.get(Calendar.MINUTE)}"
		sendEvent name: "verboseTrace", value: "get_interval> intervalHr ${intervalHr}"
		sendEvent name: "verboseTrace", value: "get_interval> intervalMin= ${intervalMin}"
	}
...
private float calculate_report_stats(component, startInterval, endInterval, typeData, operation='total') {
	...
	if (settings.trace) {
		...
		sendEvent name: "verboseTrace", value: "calculate_report_stats> total= ${total} for $component component"
		sendEvent name: "verboseTrace", value: "calculate_report_stats> nbRows with value= ${nbRows} for $component component"
		sendEvent name: "verboseTrace", value: "calculate_report_stats> avg= ${avg} for $component component"
		sendEvent name: "verboseTrace", value: "calculate_report_stats> max= ${max} for $component component"
		sendEvent name: "verboseTrace", value: "calculate_report_stats> min= ${min} for $component component"
		...
		api('thermostatInfo', bodyReq) {resp ->
			...
			if (!statusCode) {
				...
				data.thermostatList[0].settings.quickSaveSetBack = data.thermostatList[0].settings
					.quickSaveSetBack / 10
				data.thermostatList[0].settings.auxMaxOutdoorTemp = data.thermostatList[0].settings
					.auxMaxOutdoorTemp / 10
				data.thermostatList[0].settings.stage1CoolingDifferentialTemp = data.thermostatList[0].settings
					.stage1CoolingDifferentialTemp / 10
				data.thermostatList[0].settings.stage1HeatingDifferentialTemp = data.thermostatList[0].settings
...
private def thermostat_revision_changed(tstatType, thermostatId) {	
	...
	if (settings.trace) {	
		log.debug "thermostat_revision_changed>done for ${thermostatId},intervalRevision=$intervalRevision,runtimeRevision=$runtimeRevision,thermostatRevision=$thermostatRevision," +
	...
	if ((state?.runtimeRevision == runtimeRevision) &&
		...
		if (settings.trace) {	
			log.debug "thermostat_revision_changed>same revisions, no changes at ecobee for thermostatId ${thermostatId}"
		}
		return false
	} else {
		if (settings.trace) {	
			log.debug "thermostat_revision_changed>found revision changes at ecobee for thermostatId ${thermostatId} "  + 
				"state.intervalRevision=${state?.intervalRevision},state.runtimeRevision=${state?.runtimeRevision},state.thermostatRevision=${state?.thermostatRevision}"
		}
		state?.intervalRevision=intervalRevision
...
//	if device created by initialSetup (Service Manager)
	...
	if (data.auth?.thermostatId) {
		if (!parent.refreshParentTokens()) {
			...
			if (settings.trace) {
				sendEvent name: "verboseTrace", displayed: settings.trace,value:
					"refresh_tokens>warning: called refreshLocalAuthToken() after parent call failure"
				log.warn "refresh_tokens>called refreshLocalAuthToken() after parent call failure"
			}
		}            
...
private def get_stats_attribute(attribute, startDate,endDate,operation='avg',filterEvents=false,value=null) {
	...
	if ((!value) && filterEvents) {    
		count =recentStates.count {(it.value.toFloat() >MIN_TEMP_VALUE && it.value.toFloat() < MAX_TEMP_VALUE)}
	} else if (!value) {    
	...
	switch (operation) {    
		case 'total':
			if (filterEvents) {             
				result =recentStates.inject(0) { total, i -> total + ((i.value.toFloat() >MIN_TEMP_VALUE && i.value.toFloat() < MAX_TEMP_VALUE)? i.value.toFloat():0) }
			} else {
				result =recentStates.inject(0) { total, i -> total + i.value.toFloat() }
		...
		case 'deviation':
			float avgResult        
			if (filterEvents) {             
				avgResult = recentStates.inject(0) { total, i -> total + ((i.value.toFloat() >MIN_TEMP_VALUE && i.value.toFloat() < MAX_TEMP_VALUE)? i.value.toFloat():0)}
			} else {
				avgResult = recentStates.inject(0) { total, i -> total + (i.value.toFloat())}
			}            
			...
			if (filterEvents) {             
				result = recentStates.inject(0) { totalDev, i -> totalDev + ((i.value.toFloat() >MIN_TEMP_VALUE && i.value.toFloat() < MAX_TEMP_VALUE)? 
					((i.value.toFloat() - avgResult).abs()) : 0)}
			} else {  
				result = recentStates.inject(0) { totalDev, i -> totalDev + ((i.value.toFloat() - avgResult).abs())}
			}			                    
			result = (count)? (result /count).round(2):0        
			if (settings.trace) {            
				log.debug("get_stats_attribute>for ${attribute}, avg=${avgResult},avgDeviation=${result},count=${count}")           
				sendEvent  name: "verboseTrace", value: ("get_stats_attribute>for ${attribute}, avg=${avgResult},avgDeviation=${result},count=${count}")           
			}                
			break            
		case 'min':
			if (filterEvents) {             
				result =recentStates.inject(MAX_TEMP_VALUE) {min, i -> Math.min(min,((i.value.toFloat() >MIN_TEMP_VALUE && i.value.toFloat() < MAX_TEMP_VALUE)?i.value.toFloat():MAX_TEMP_VALUE))}
			} else {
				result =recentStates.inject(MAX_TEMP_VALUE) {min, i -> Math.min(min,(i.value.toFloat())) }
			}            
			break            
		case 'max':
			if (filterEvents) {             
				result =recentStates.inject(MIN_TEMP_VALUE) {max, i -> Math.max(max,((i.value.toFloat() >MIN_TEMP_VALUE && i.value.toFloat() < MAX_TEMP_VALUE)?i.value.toFloat():MIN_TEMP_VALUE)) }
			} else {
				result =recentStates.inject(MIN_TEMP_VALUE) {max, i -> Math.max(max,(i.value.toFloat())) }
			}            
			break            
		case 'values':
        
			if (filterEvents) {             
				result =recentStates.inject([]) {valueSet, i -> (i.value.toFloat() >MIN_TEMP_VALUE && i.value.toFloat() < MAX_TEMP_VALUE)?
					valueSet << i.value: {} }
			} else if (value) {
		...
		case 'dates':
			if (filterEvents) {             
				...
				if (settings.trace) {                
					valuesFound.each {                
						log.debug("get_stats_attribute>dates valueFound= ${it.value}, date=${it.date.getTime()}")
						sendEvent  name: "verboseTrace", value: "get_stats_attribute>dates valueFound= ${it.value}, date=${it.date.getTime()}"
					}                        
				}                    
				result = valuesFound.inject([])  {valueSet, i -> valueSet << i.date.getTime()}               
				if (settings.trace) {                
					log.debug("get_stats_attribute>dates result set= $result")
					sendEvent  name: "verboseTrace", value: "get_stats_attribute>dates result set= ${result}"
				}                    
			} else {                 
...
private def get_recommendation(tip) {
	...
	def tips = [
		...
		'tip07': 
			[level:1, 
				...
				"For example if you select Synchronize User Preferences, when you configure any user preference on one thermostat (i.e. choose Celsius vs. Fahrenheit) ALL thermostats within that group will change to Celsius."			
			],
		'tip10': 
			[level:2, 
		...
		'tip25': 
			[level:3, 
				text:"The auxOutdoorTemp parameter is found in Installation Settings,Thresholds, and finally Aux Heat Settings. This is the temperature at which ecobee will not run auxiliary Heat under any circumstances. " +
	...
	if (settings.trace) {
		log.debug "get_recommendation_text>got ${tip}'s text from tips table"
	}
...
private void initialize_tips() {
	for (int i=1;i<=get_MAX_TIPS();i++) {    
		...
		sendEvent name: attribute, value: "",displayed: settings.trace
...
void resetTips() {
	if (settings.trace) {
		log.debug "resetTips>begin with state?.tips=${state.tips}, about to reset the state variable"
	}        
	state?.tips=[] // reset the state variable which saves all tips previously given
}

void getTips(level=5) {
	...
	if (settings.trace) {
		log.debug "getTips>begin with level=$level"
		sendEvent name: "verboseTrace", value: "getTips>begin with level=$level",
           	displayed: settings.trace
		log.debug "getTips>tipsAlreadyGiven=$tipsAlreadyGiven, state?.tips=${state.tips}"
		sendEvent name: "verboseTrace", value: "getTips>tipsAlreadyGiven=$tipsAlreadyGiven,state.tips=${state?.tips}",
          	displayed: settings.trace
	...
	def avgCoolingSetpoint,devCoolingSetpoint,minCoolingSetpoint,maxCoolingSetpoint
	def avgHeatingSetpoint,devHeatingSetpoint,minHeatingSetpoint,maxHeatingSetpoint
	...
	def avgTemperature=get_stats_attribute("temperature",aWeekAgo,endDate,'avg',true)
	def devTemperature=get_stats_attribute("temperature",aWeekAgo,endDate,'deviation',true)
	...
	def awayValues=get_stats_attribute("setClimate",startDate,endDate,'values',false,'Away')
	def sleepValues=get_stats_attribute("setClimate",startDate,endDate,'values',false,'Sleep')
	def remoteMinTemp = get_stats_attribute("remoteSensorMinTemp",aWeekAgo,endDate,'min',true)     
	def remoteMaxTemp = get_stats_attribute("remoteSensorMaxTemp",aWeekAgo,endDate,'max',true)     
	def remoteAvgTemp = get_stats_attribute("remoteSensorAvgTemp",aWeekAgo,endDate,'avg',true)
	...
	if (settings.trace) {    
		log.debug ("getTips>currentTemp = $currentTemp")            
		sendEvent  name: "verboseTrace", value: "getTips>currentTemp = $currentTemp"
		log.debug ("getTips>currentIndoorHum = $currentIndoorHum")            
		sendEvent  name: "verboseTrace", value:"getTips>currentIndoorHum = $currentIndoorHum"            
		log.debug ("getTips>targetHumidity=$targetHumidity")
		sendEvent  name: "verboseTrace",value:"getTips>targetHumidity=$targetHumidity"
		log.debug ("getTips>avgTemperature=$avgTemperature")    
		sendEvent  name: "verboseTrace", value: "getTips>avgTemperature=$avgTemperature"    
		log.debug ("getTips>outdoorTemp=$outdoorTemp")
		sendEvent  name: "verboseTrace", value:"getTips>outdoorTemp=$outdoorTemp"
		log.debug ("getTips>devTemperature=$devTemperature")    
		sendEvent  name: "verboseTrace", value: "getTips>devTemperature=$devTemperature"
		log.debug ("getTips>avgHeatingSetpoint=$avgHeatingSetpoint")  
		sendEvent name: "verboseTrace", value: "getTips>avgHeatingSetpoint=$avgHeatingSetpoint"
		log.debug ("getTips>devHeatingSetpoint=$devHeatingSetpoint")  
		sendEvent  name: "verboseTrace", value: "getTips>avgcoolingSetpoint=$avgCoolingSetpoint"
		log.debug ("getTips>avgcoolingSetpoint=$avgCoolingSetpoint")  
		log.debug ("getTips>countCooling=$countCooling")  
		sendEvent  name: "verboseTrace", value: "getTips>countCooling=$countCooling"
		log.debug ("getTips>countHeating=$countHeating")  
		sendEvent name: "verboseTrace", value: "getTips>countHeating=$countHeating"
		log.debug ("getTips>countHolds=$countHolds")  
		sendEvent name: "verboseTrace", value: "getTips>countHolds=$countHolds"
		log.debug ("getTips>coolingSetPoint values=$coolingValuesSet")  
		sendEvent  name: "verboseTrace",value: "getTips>coolingSetPoint values=$coolingValuesSet"
		log.debug ("getTips>heatingSetPoint values=$heatingValuesSet")  
		sendEvent  name: "verboseTrace",value: "getTips>heatingSetPoint values=$heatingValuesSet"
		log.debug ("getTips>min heatingSetpoint=$minHeatingSetpoint")
		sendEvent  name: "verboseTrace",value: "getTips>min heatingSetpoint=$minHeatingSetpoint"
		log.debug ("getTips>min coolingSetpoint=$minCoolingSetpoint")
		sendEvent  name: "verboseTrace",value: "getTips>max coolingSetpoint=$maxCoolingSetpoint"
		log.debug ("getTips>max heatingSetpoint=$maxHeatingSetpoint")
		sendEvent  name: "verboseTrace",value: "getTips>max heatingSetpoint=$maxHeatingSetpoint"
		log.debug ("getTips>awayValues=$awayValues")  
		sendEvent  name: "verboseTrace", value: "getTips>awayValues=$awayValues"
		log.debug ("getTips>sleepValues=$sleepValues")  
		sendEvent  name: "verboseTrace", value: "getTips>sleepValues=$sleepValues"
		log.debug ("getTips>awayDates=$awayDates")  
		sendEvent  name: "verboseTrace", value: "getTips>awayDates=$awayDates"
		log.debug ("getTips>sleepDates=$sleepDates")  
		sendEvent  name: "verboseTrace", value: "getTips>sleepDates=$sleepDates"
		log.debug ("getTips>sleepDuration=$sleepDuration")  
		sendEvent  name: "verboseTrace", value: "getTips>sleepDuration=$sleepDuration"
		log.debug ("getTips>awayDuration=$awayDuration")  
		sendEvent  name: "verboseTrace", value: "getTips>awayDuration=$awayDuration"
		log.debug ("getTips>homeDates=$homeDates")  
		sendEvent  name: "verboseTrace", value: "getTips>homeDates=$homeDates"
		log.debug ("getTips>countAway=$countAway")  
		sendEvent  name: "verboseTrace", value: "getTips>countAway=$countAway"
		log.debug ("getTips>countSleep=$countSleep")  
		sendEvent  name: "verboseTrace", value: "getTips>countSleep=$countSleep"
		log.debug ("getTips>remoteMinTemp=$remoteMinTemp")
		sendEvent  name: "verboseTrace",value:"getTips>remoteMinTemp=$remoteMinTemp"
		log.debug ("getTips>remoteMaxTemp=$remoteMaxTemp")        	
		sendEvent  name: "verboseTrace",value:"getTips>remoteMaxTemp=$remoteMaxTemp"        	
		log.debug ("getTips>remoteAvgTemp=$remoteAvgTemp") 
		sendEvent  name: "verboseTrace",value:"getTips>remoteAvgTemp=$remoteAvgTemp" 
		log.debug ("getTips>coolStages=$coolStages")
		sendEvent  name: "verboseTrace",value:"getTips>coolStages=$coolStages"
		log.debug ("getTips>heatStages=$heatStages")
		sendEvent  name: "verboseTrace",value:"getTips>heatStages=$heatStages"
	}   
	if (level <=1) {
		if (!tipsAlreadyGiven.contains("tip01")) {
			if (countHeating && !countSleep) {
				...
				sendEvent name: attribute, value: "Observation: Your thermostat's minimum cooling setpoint of ${minHeatingSetpoint} degrees could be decreased at Nights. Current tip is: " + recommendation.text
					displayed: settings.trace
				attribute ="tip${state?.currentTip}Level"                
				sendEvent name: attribute, value: recommendation.level,displayed: settings.trace
				attribute ="tip${state?.currentTip}Solution"                
				sendEvent name: attribute, value: "tip01,heating",displayed: settings.trace
			}        
			if ((!tipsAlreadyGiven.contains("tip01")) && countCooling && !countSleep) {
				...
				state?.currentTip=state?.currentTip +1
				attribute ="tip${state?.currentTip}Text"                
				sendEvent name: attribute, value: "Observation: Your thermostat's minimum cooling setpoint of ${maxCoolingSetpoint} degrees could be increased at Nights. Current tip is: " + recommendation.text,
					displayed: settings.trace
				attribute ="tip${state?.currentTip}Level"                
				sendEvent name: attribute, value: recommendation.level,displayed: settings.trace
				attribute ="tip${state?.currentTip}Solution"                
				sendEvent name: attribute, value: "tip01,cooling",displayed: settings.trace
			}        
		} /* end of tip01 logic */
		...
		if ((isWeekday) && (!tipsAlreadyGiven.contains("tip02"))) {
			if ((mode in ['heat','auto', 'off']) && countHeating && !countAway) {
				...
				sendEvent name: attribute, value: recommendation.level,displayed: settings.trace
				attribute ="tip${state?.currentTip}Solution"                
				sendEvent name: attribute, value: "tip02,heating",displayed: settings.trace
			}        
			if (((!tipsAlreadyGiven.contains("tip02")) && (mode in ['cool','auto', 'off']) && countCooling) && !countAway) {
				recommendation= get_recommendation('tip02')
				tipsAlreadyGiven.add('tip02')                
				state?.currentTip=state?.currentTip +1             
				attribute ="tip${state?.currentTip}Text"                
				sendEvent name: attribute, value: "Observation: Your thermostat's minimum cooling setpoint of ${maxCoolingSetpoint} degrees could be increased during workdays when away for work. Current tip is: " + recommendation.text,
					displayed: settings.trace
				attribute ="tip${state?.currentTip}Level"                
				sendEvent name: attribute, value: recommendation.level,displayed: settings.trace
				attribute ="tip${state?.currentTip}Solution"                
				sendEvent name: attribute, value: "tip02,cooling",displayed: settings.trace
			}        
		} /* end of tip02 logic */
		if (devTemperature.toFloat() >=MAX_DEVIATION_TEMPERATURE) {
			...
			attribute ="tip${state?.currentTip}Level"                
			sendEvent name: attribute, value: recommendation.level,displayed: settings.trace
			if ((!tipsAlreadyGiven.contains("tip03")) && (remoteMinTemp && remoteAvgTemp)) {
				if ((((remoteMinTemp.toFloat() - remoteAvgTemp.toFloat()).abs()) > MAX_DIFFERENCE_TEMPERATURE) ||
					...
					sendEvent name: attribute, value: recommendation.level,displayed: settings.trace
					attribute ="tip${state?.currentTip}Solution"                
					sendEvent name: attribute, value: "tip03",displayed: settings.trace
				} else if ((!tipsAlreadyGiven.contains("tip04")) && (remoteMinTemp && remoteAvgTemp)) {
					...
					sendEvent name: attribute, value: recommendation.level,displayed: settings.trace
					attribute ="tip${state?.currentTip}Solution"                
					sendEvent name: attribute, value: "tip04",displayed: settings.trace
				} /* end tip 04 */
			}            
		} 
		if ((state?.currentTip < maxTips) && (countHolds >= MAX_HOLD_THRESHOLD) && (!tipsAlreadyGiven.contains("tip05"))) {
			...
			sendEvent name: attribute, value: recommendation.level,displayed: settings.trace
			attribute ="tip${state?.currentTip}Solution"                
			sendEvent name: attribute, value: "tip05",displayed: settings.trace
		} /* end of tip05 logic */
		if ((state?.currentTip < maxTips) && (countHolds >= MAX_HOLD_THRESHOLD) && (!tipsAlreadyGiven.contains("tip06"))) {
			...
			sendEvent name: attribute, value: recommendation.level,displayed: settings.trace
			attribute ="tip${state?.currentTip}Solution"                
			sendEvent name: attribute, value: "tip06",displayed: settings.trace
		} /* end of tip06 logic */
		if ((state?.currentTip < maxTips) && (data.thermostatCount > 1) && (!tipsAlreadyGiven.contains("tip07"))) {
			...
			sendEvent name: attribute, value: recommendation.level,displayed: settings.trace
			attribute ="tip${state?.currentTip}Solution"                
			sendEvent name: attribute, value: "tip07",displayed: settings.trace
        
		}        
		state?.tips=tipsAlreadyGiven    
		if (settings.trace) {
			log.debug "getTips>end Level 1 with tipsAlreadyGiven=$tipsAlreadyGiven, state?.tips=${state.tips}"
			sendEvent name: "verboseTrace", value: "getTips>end Level 1 with tipsAlreadyGiven=$tipsAlreadyGiven,state.tips=${state?.tips}"
		}   	     
	} /* end of level 1 */    
    

	if (level <=2 ) {
		if ((state?.currentTip < maxTips) && (!tipsAlreadyGiven.contains("tip10")) && countHeating && ((maxHeatingSetpoint.toFloat() -minHeatingSetpoint.toFloat()) <= MIN_DEVIATION_HEATING_SETPOINT)) {
			...
			sendEvent name: attribute, value: recommendation.level,displayed: settings.trace
			attribute ="tip${state?.currentTip}Solution"                
			sendEvent name: attribute, value: "tip10,heating",displayed: settings.trace
		}        
		if ((state?.currentTip < maxTips) && (!tipsAlreadyGiven.contains("tip10")) && countCooling && ((maxCoolingSetpoint.toFloat() -minCoolingSetpoint.toFloat()) <= MIN_DEVIATION_COOLING_SETPOINT)) {
			recommendation= get_recommendation('tip10')
			tipsAlreadyGiven.add('tip10')                
			state?.currentTip=state?.currentTip +1             
			attribute ="tip${state?.currentTip}Text"                
			sendEvent name: attribute, value: "Observation: There is a not a big difference between your thermostat's minimum cooling setpoint of ${minCoolingSetpoint} and your maximum heating setpoint of ${maxCoolingSetpoint} degrees in the last 24 hours. Current tip is: " + recommendation.text,
				displayed: settings.trace
			attribute ="tip${state?.currentTip}Level"                
			sendEvent name: attribute, value: recommendation.level,displayed: settings.trace
			attribute ="tip${state?.currentTip}Solution"                
			sendEvent name: attribute, value: "tip10,cooling",displayed: settings.trace
		} /* end of tip10 logic */
        
		if ((state?.currentTip < maxTips) && !tipsAlreadyGiven.contains("tip11") ) {
			if ((countHeating) && (countHeating.toInteger() >= MAX_HEATING_CYCLE)) {
				...
				sendEvent name: attribute, value: recommendation.level,displayed: settings.trace
				attribute ="tip${state?.currentTip}Solution"                
				sendEvent name: attribute, value: "tip11,heating",displayed: settings.trace
            
			} 
			if ((state?.currentTip < maxTips) && ((countCooling) && 
  				(countCooling.toInteger() >= MAX_COOLING_CYCLE))) {
				recommendation= get_recommendation('tip11')
				tipsAlreadyGiven.add('tip11')                
				state?.currentTip=state?.currentTip +1             
				attribute ="tip${state?.currentTip}Text"                
				...
				sendEvent name: attribute, value: recommendation.level,displayed: settings.trace
				attribute ="tip${state?.currentTip}Solution"                
				sendEvent name: attribute, value: "tip11,cooling",displayed: settings.trace
            
			}            
        
		} /* end of tip11 logic */
		if ((state?.currentTip < maxTips) && ((fanRuntime) && (fanRuntime < MINIMUM_FAN_RUNTIME)) &&  (!tipsAlreadyGiven.contains("tip14")) && (remoteMinTemp && remoteAvgTemp))  {
			float delta_temp_max = (remoteMaxTemp.toFloat() - remoteAvgTemp.toFloat()).abs().round(1)                
			float delta_temp_min = (remoteMinTemp.toFloat() - remoteAvgTemp.toFloat()).abs().round(1)                
			if ((delta_temp_max > MAX_DIFFERENCE_TEMPERATURE) ||
				...
				sendEvent name: attribute, value: "Observation: Your Fan Runtime was ${fanRuntime} minutes yesterday, and you have some wide temperature deltas (+${delta_temp_max}/-${delta_temp_min} degrees) vs. the average of ${remoteAvgTemp} degrees " + 
					"between all your remote sensors. Current tip is: " + recommendation.text,displayed: settings.trace
				attribute ="tip${state?.currentTip}Level"                
				sendEvent name: attribute, value: recommendation.level,displayed: settings.trace
				attribute ="tip${state?.currentTip}Solution"                
				sendEvent name: attribute, value: "tip14",displayed: settings.trace
    
			}  		  
		}  		  
     
		if ((state?.currentTip < maxTips) && (!tipsAlreadyGiven.contains("tip15")) && (currentIndoorHum > (targetHumidity + HUMIDITY_DIFF_ALLOWED))) {
			...
			sendEvent name: attribute, value: recommendation.level,displayed: settings.trace
			attribute ="tip${state?.currentTip}Solution"                
			sendEvent name: attribute, value: "tip15",displayed: settings.trace
		...
		if ((state?.currentTip < maxTips) &&  (!tipsAlreadyGiven.contains("tip16")) && (sleepDuration) && (sleepDuration.toFloat() < MIN_USUAL_SLEEP_DURATION)) {
			...
			sendEvent name: attribute, value: recommendation.level,displayed: settings.trace
			attribute ="tip${state?.currentTip}Solution"                
			sendEvent name: attribute, value: "tip16",displayed: settings.trace
		...
		if ((isWeekday) && (state?.currentTip < maxTips) &&  (!tipsAlreadyGiven.contains("tip17")) && (awayDuration) && (awayDuration.toFloat() < MIN_USUAL_AWAY_DURATION)) {
			...
			sendEvent name: attribute, value: recommendation.level,displayed: settings.trace
			attribute ="tip${state?.currentTip}Solution"                
			sendEvent name: attribute, value: "tip17",displayed: settings.trace
		}  	/* end of tip17 logic */  	
		...
		if ((state?.currentTip < maxTips) &&  (!tipsAlreadyGiven.contains("tip18")) && (awayDuration) && (awayDuration.toFloat() > MAX_USUAL_AWAY_DURATION)) {
			...
			sendEvent name: attribute, value: recommendation.level,displayed: settings.trace
			attribute ="tip${state?.currentTip}Solution"                
			sendEvent name: attribute, value: "tip18",displayed: settings.trace
		}  	/* end of tip18 logic */  	
        
		state?.tips=tipsAlreadyGiven    
		if (settings.trace) {
			log.debug "getTips>end Level 2 with tipsAlreadyGiven=$tipsAlreadyGiven, state?.tips=${state.tips}"
			sendEvent name: "verboseTrace", value: "getTips>end Level 2 with tipsAlreadyGiven=$tipsAlreadyGiven,state.tips=${state?.tips}"
		}   	     
        
	}   /* end level 2 */  
    
	if (state?.currentTip < maxTips && (level <=3)) {
		tipsAlreadyGiven=get_tips_level3()    
		state?.tips = tipsAlreadyGiven 
	}  
    
	if (((state?.currentTip < maxTips)  && (level <=4)) && ((heatStages>1) ||  (coolStages>1))) {
		tipsAlreadyGiven=get_tips_level4()    
		state?.tips = tipsAlreadyGiven 
	}    
	if (settings.trace) {
		log.debug "getTips>end with tipsAlreadyGiven=$tipsAlreadyGiven, state?.tips=${state.tips}"
		sendEvent name: "verboseTrace", value: "getTips>end with tipsAlreadyGiven=$tipsAlreadyGiven,state.tips=${state?.tips}"
	}        
...
private def get_tips_level3() {
	...
	def tipsAlreadyGiven = (state?.tips != [])? state.tips: []
	if (settings.trace) {
		log.debug "get_tips_level3>begin with tipsAlreadyGiven=$tipsAlreadyGiven, state?.tips=${state.tips}"
		sendEvent name: "verboseTrace", value: "get_tips_level3>begin with tipsAlreadyGiven=$tipsAlreadyGiven,state.tips=${state?.tips}"
	}        
	def recommendation=null
	...
	if ((state?.currentTip < maxTips) && (!tipsAlreadyGiven.contains("tip20")) && (hasHumidifier=='true') && 
		...
		sendEvent name: attribute, value: recommendation.level,	displayed: settings.trace                
		attribute ="tip${state?.currentTip}Solution"                
		sendEvent name: attribute, value: "tip20",displayed: settings.trace
	}    /* end of tip20 logic */   
	if ((state?.currentTip < maxTips) && (!tipsAlreadyGiven.contains("tip21")) && (hasDehumidifier =='true') && (currentIndoorHum > (targetHumidity + HUMIDITY_DIFF_ALLOWED))) {
		...
		sendEvent name: attribute, value: recommendation.level,displayed: settings.trace
		attribute ="tip${state?.currentTip}Solution"                
		sendEvent name: attribute, value: "tip21",displayed: settings.trace
	}  	/* end of tip21 logic */  	  
	if ((state?.currentTip < maxTips) &&  (!tipsAlreadyGiven.contains("tip22")) && (hasErv=='true' || hasHrv =='true') && (currentIndoorHum > (targetHumidity + HUMIDITY_DIFF_ALLOWED))) {
		...
		sendEvent name: attribute, value: recommendation.level,displayed: settings.trace
		attribute ="tip${state?.currentTip}Solution"                
		sendEvent name: attribute, value: "tip22",displayed: settings.trace
    
	}  else if ((state?.currentTip < maxTips) &&  (!tipsAlreadyGiven.contains("tip23")) && (hasErv=='true' || hasHrv =='true') && (currentIndoorHum > (targetHumidity + HUMIDITY_DIFF_ALLOWED))) {
		...
		sendEvent name: attribute, value: recommendation.level,displayed: settings.trace
		attribute ="tip${state?.currentTip}Solution"                
		sendEvent name: attribute, value: "tip23",displayed: settings.trace
    
	}  	  /* end of tip23 logic */  	  
	if ((state?.currentTip < maxTips) && (!tipsAlreadyGiven.contains("tip24")) && (hasErv!='true' && hasHrv !='true') && (countCooling) && (currentIndoorHum > (targetHumidity + HUMIDITY_DIFF_ALLOWED))) { 
		...
		sendEvent name: attribute, value: recommendation.level,displayed: settings.trace
		attribute ="tip${state?.currentTip}Solution"                
		sendEvent name: attribute, value: "tip24",displayed: settings.trace
	} 	/* end of tip24 logic */                  
	if ((state?.currentTip < maxTips) && (!tipsAlreadyGiven.contains("tip25")) && (heatStages>1)) { 
		...
		state?.currentTip=state?.currentTip +1             
		attribute ="tip${state?.currentTip}Text"                
		sendEvent name: attribute, value: "Observation: You have a ${heatStages}-stage heating system and your auxOutdoorTemp setting is ${auxOutdoorTemp}." +  
			"Current tip is: " + recommendation.text,displayed: settings.trace
             
		attribute ="tip${state?.currentTip}Level"                
		sendEvent name: attribute, value: recommendation.level,displayed: settings.trace
		attribute ="tip${state?.currentTip}Solution"                
		sendEvent name: attribute, value: "tip25",displayed: settings.trace
	...
	if ((state?.currentTip < maxTips) && (!tipsAlreadyGiven.contains("tip26")) && (heatStages>1)) { 
		...
		sendEvent name: attribute, value: "Observation: You have a ${heatStages}-stage heating system. Here are some of your current settings for auxHeat: stage1HeatingDifferentialTemp=${stage1HeatingDifferentialTemp} " +  
			"which is the difference between current temperature and set-point that will trigger stage-2 heating and stage1HeatingDissipationTimes=${stage1HeatingDissipationTime} seconds, which is the time after a heating cycle that the fan will run for to extract any heating left in the system - 30 second default." + 
			"Current tip is: " + recommendation.text,displayed: settings.trace
		attribute ="tip${state?.currentTip}Level"                
		sendEvent name: attribute, value: recommendation.level,displayed: settings.trace
		attribute ="tip${state?.currentTip}Solution"                
		sendEvent name: attribute, value: "tip26",displayed: settings.trace
	...
	if ((state?.currentTip < maxTips) && (devOutdoorTemperature.toFloat() <= MAX_DEVIATION_OUTDOOR_TEMP) && (dailyCoolingRuntime)) {
		if ((!tipsAlreadyGiven.contains("tip30")) && ((yesterdayCoolingRuntime && avgWeeklyCoolingRuntime) && 
			...
			sendEvent name: attribute, value: recommendation.level,displayed: settings.trace
			attribute ="tip${state?.currentTip}Solution"                
			sendEvent name: attribute, value: "tip30,cooling",displayed: settings.trace
		}
		if ((!tipsAlreadyGiven.contains("tip30")) && ((yesterdayHeatingRuntime && avgWeeklyHeatingRuntime) && 
			...
			sendEvent name: attribute, value: recommendation.level,displayed: settings.trace
			attribute ="tip${state?.currentTip}Solution"                
			sendEvent name: attribute, value: "tip30,heating",displayed: settings.trace
		}
		if ((state?.currentTip < maxTips) && (!tipsAlreadyGiven.contains("tip31")) && ((yesterdayCoolingRuntime) && 
			...
			sendEvent name: attribute, value: recommendation.level,displayed: settings.trace
			attribute ="tip${state?.currentTip}Solution"                
			sendEvent name: attribute, value: "tip31,cooling",displayed: settings.trace
		}              
		if ((state?.currentTip < maxTips) && (!tipsAlreadyGiven.contains("tip31")) && ((yesterdayHeatingRuntime) &&  
			...
			sendEvent name: attribute, value: recommendation.level,displayed: settings.trace
			attribute ="tip${state?.currentTip}Solution"                
			sendEvent name: attribute, value: "tip31,heating",displayed: settings.trace
		} /* end of tip31 logic */       
		...
		if ((state?.currentTip < maxTips) && (!tipsAlreadyGiven.contains("tip32")) && ((yesterdayCoolingRuntime && avgWeeklyCoolingRuntime) && 
			...
			sendEvent name: attribute, value: recommendation.level,displayed: settings.trace
			attribute ="tip${state?.currentTip}Solution"                
			sendEvent name: attribute, value: "tip32",displayed: settings.trace
		}
	}        
	if (settings.trace) {
		log.debug "get_tips_level3>end with tipsAlreadyGiven=$tipsAlreadyGiven, state?.tips=${state.tips}"
		sendEvent name: "verboseTrace", value: "get_tips_level3>end with tipsAlreadyGiven=$tipsAlreadyGiven,state.tips=${state?.tips}"
	}        
...
private def get_tips_level4() {
	...
	def maxTips=get_MAX_TIPS()    
	if (settings.trace) {
		log.debug "get_tips_level4>begin with tipsAlreadyGiven=$tipsAlreadyGiven, state?.tips=${state.tips}"
		sendEvent name: "verboseTrace", value: "get_tips_level4>begin with tipsAlreadyGiven=$tipsAlreadyGiven,state.tips=${state?.tips}"
	}        
	def recommendation=null
	def attribute,component   
	boolean isWeekday=isWeekday()    
...
*/    
	...
	if (heatStages>1) { 
		...
		if (settings.trace) {
			log.debug ("get_tips_level4>heatAuxRuntimeDaily=$heatAuxRuntimeDaily minutes")
			sendEvent  name: "verboseTrace",value:"get_tips_level4>heatAuxRuntimeDaily=$heatAuxRuntimeDaily minutes"
			log.debug ("get_tips_level4>heatAuxRuntimeYesterday=$heatAuxRuntimeYesterday minutes")
			sendEvent  name: "verboseTrace", value: "get_tips_level4>heatAuxRuntimeYesterday=$heatAuxRuntimeYesterday minutes"
			log.debug ("get_tips_level4>heatAuxRuntimeAvgWeekly=$heatAuxRuntimeAvgWeekly minutes")
			sendEvent  name: "verboseTrace",value:"get_tips_level4>heatAuxRuntimeAvgWeekly=$heatAuxRuntimeAvgWeekly minutes"
			log.debug ("get_tips_level4>heatAuxRuntimeAvgMonthly=$heatAuxRuntimeAvgMonthly minutes")
		...
		if (settings.trace) {
			log.debug ("get_tips_level4>ratioAuxHeatDaily=$ratioAuxDaily %")
			sendEvent  name: "verboseTrace",value:"get_tips_level4>ratioAuxHeatDaily=$ratioAuxDaily %"
		}            
		float ratioAuxYesterday=((heatAuxRuntimeYesterday/yesterdayHeatingRuntime)*100).round(1)
		if (settings.trace) {
			log.debug ("get_tips_level4>ratioAuxHeatYesterday=$ratioAuxHeatYesterday %")
			sendEvent  name: "verboseTrace",value:"get_tips_level4>ratioAuxHeatYesterday=$ratioAuxYesterday %"
		}            
		float ratioAuxWeekly=((heatAuxRuntimeWeekly/avgWeeklyHeatingRuntime)*100).round(1)
		if (settings.trace) {
			log.debug ("get_tips_level4>ratioAuxHeatWeekly=$ratioAuxWeekly %")
			sendEvent  name: "verboseTrace",value:"get_tips_level4>ratioAuxHeatWeekly=$ratioAuxWeekly %"
		}            
		float ratioAuxMonthly=((heatAuxRuntimeMonthly/avgMonthlyHeatingRuntime)*100).round(1)
		if (settings.trace) {
			log.debug ("get_tips_level4>ratioAuxHeatMonthly=$ratioAuxMonthly %")
			sendEvent  name: "verboseTrace",value:"get_tips_level4>ratioAuxHeatMonthly=$ratioAuxMonthly %"
		}
		if ((state?.currentTip < maxTips) && (!tipsAlreadyGiven.contains("tip40")) &&  (dailyHeatingRuntime) && (((avgOutdoorTemperature > LOW_TEMP_THRESHOLD_FOR_HEATING)) &&
			...
			sendEvent name: attribute, value: recommendation.level,displayed: settings.trace
			attribute ="tip${state?.currentTip}Solution"                
			sendEvent name: attribute, value: "tip40",displayed: settings.trace
		} /* end of tip40 logic */                   
		if ((state?.currentTip < maxTips) && (!tipsAlreadyGiven.contains("tip41")) &&  (yesterdayHeatingRuntime) && (ratioAuxDaily < ratioAuxYesterday)) {            
			...
			sendEvent name: attribute, value: recommendation.level,displayed: settings.trace
			attribute ="tip${state?.currentTip}Solution"                
			sendEvent name: attribute, value: "tip41",displayed: settings.trace
		} /* end of tip41 logic */                   
		if ((state?.currentTip < maxTips) && (!tipsAlreadyGiven.contains("tip42")) &&  (avgWeeklyHeatingRuntime) && (ratioAuxDaily < ratioAuxWeekly)) {            
			...
			sendEvent name: attribute, value: recommendation.level,displayed: settings.trace
			attribute ="tip${state?.currentTip}Solution"                
			sendEvent name: attribute, value: "tip42",displayed: settings.trace
		} /* end of tip42 logic */                   
		if ((state?.currentTip < maxTips) && (!tipsAlreadyGiven.contains("tip43")) &&  (avgMonthlyHeatingRuntime) && (ratioAuxDaily < ratioAuxMonthly)) {            
			...
			sendEvent name: attribute, value: recommendation.level,displayed: settings.trace
			attribute ="tip${state?.currentTip}Solution"                
			sendEvent name: attribute, value: "tip43",displayed: settings.trace
		} /* end of tip43 logic */       
	} /* end if heatStages */        
	...
	if (coolStages>1) { 
		if (settings.trace) {
			log.debug ("get_tips_level4>About to calculate Aux Cooling ratios")
			sendEvent  name: "verboseTrace",value: "get_tips_level4>About to calculate Aux Cooling ratios"
		}   
		...
		if (settings.trace) {
			log.debug ("get_tips_level4>ratioAuxCoolDaily=$ratioAuxDaily %")
			sendEvent  name: "verboseTrace",value:"get_tips_level4>ratioAuxCoolDaily=$ratioAuxDaily %"
		}        
		if (yesterdayCoolingRuntimeCool1) {
			ratioAuxYesterday=((yesterdayCoolingRuntimeCool1/yesterdayCoolingRuntime)*100).round(1)
		}            
		if (settings.trace) {
			log.debug ("get_tips_level4>ratioAuxCoolYesterday=$ratioAuxYesterday %")
			sendEvent  name: "verboseTrace",value:"get_tips_level4>ratioAuxCoolYesterday=$ratioAuxYesterday %"
		}        
		if (avgWeeklyCoolingRuntimeCool1) {
			ratioAuxWeekly=((avgWeeklyCoolingRuntimeCool1/avgWeeklyCoolingRuntime)*100).round(1)
		}            
		if (settings.trace) {
			log.debug ("get_tips_level4>ratioAuxCoolWeekly=$ratioAuxWeekly %")
			sendEvent  name: "verboseTrace",value:"get_tips_level4>ratioAuxCoolWeekly=$ratioAuxWeekly %"
		...
		if (settings.trace) {
			log.debug ("get_tips_level4>ratioAuxCoolMonthly=$ratioAuxMonthly %")
			sendEvent  name: "verboseTrace",value:"get_tips_level4>ratioAuxCoolMonthly=$ratioAuxMonthly %"
		}        
		...
		if ((state?.currentTip < maxTips) && (!tipsAlreadyGiven.contains("tip44")) &&  (dailyCoolingRuntime) && (((avgOutdoorTemperature > HIGH_TEMP_THRESHOLD_FOR_COOLING)) &&
			...
			sendEvent name: attribute, value: recommendation.level,displayed: settings.trace
			attribute ="tip${state?.currentTip}Solution"                
			sendEvent name: attribute, value: "tip44",displayed: settings.trace
		} /* end of tip44 logic */                   
		if ((state?.currentTip < maxTips) && (!tipsAlreadyGiven.contains("tip45")) && (yesterdayCoolingRuntime) && (ratioAuxDaily < ratioAuxYesterday)) {            
			...
			sendEvent name: attribute, value: recommendation.level,displayed: settings.trace
			attribute ="tip${state?.currentTip}Solution"                
			sendEvent name: attribute, value: "tip45",displayed: settings.trace
		} /* end of tip45 logic */                   
		if ((state?.currentTip < maxTips) && (!tipsAlreadyGiven.contains("tip46")) &&  (avgWeeklyCoolingRuntime) && (ratioAuxDaily < ratioAuxWeekly)) {            
			...
			sendEvent name: attribute, value: recommendation.level,displayed: settings.trace
			attribute ="tip${state?.currentTip}Solution"                
			sendEvent name: attribute, value: "tip46",displayed: settings.trace
		} /* end of tip46 logic */                   
		if ((state?.currentTip < maxTips) && (!tipsAlreadyGiven.contains("tip47")) &&  (avgMonthlyCoolingRuntime) && (ratioAuxDaily < ratioAuxMonthly)) {            
			...
			sendEvent name: attribute, value: recommendation.level, displayed: settings.trace
			attribute ="tip${state?.currentTip}Solution"                
			sendEvent name: attribute, value: "tip47",displayed: settings.trace
		} /* end of tip47 logic */                   
        
	}    
	if (settings.trace) {
		log.debug "get_tips_level4>end with tipsAlreadyGiven=$tipsAlreadyGiven, state?.tips=${state.tips}"
		sendEvent name: "verboseTrace", value: "get_tips_level4>end with tipsAlreadyGiven=$tipsAlreadyGiven,state.tips=${state?.tips}"
	}        
	return tipsAlreadyGiven    
